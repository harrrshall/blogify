    ...
    const stream = new ReadableStream({
      async start(controller) {
        try {
          ...
          await setCachedResult(cacheKey, result.blogPost);

          // Push the final result to the client
          controller.enqueue(new TextEncoder().encode(JSON.stringify({ blogPost: result.blogPost, progress: 100, cached: false })));
          controller.close();
        } catch (error) {
